...
async function saveHistory(env, userId, history) {
  try {
    await env.CHAT_HISTORY.put(`history:${userId}`, JSON.stringify(history), {
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/reset" && request.method === "POST") {
      ...
      await env.CHAT_HISTORY.delete(`history:${body.user_id}`);
      return json({ ok: true });
    }
    ...
    if (url.pathname === "/imbot" && request.method === "POST") {
      ...
      const botId   = data["data[PARAMS][BOT_ID]"];
      ...
      if (message === "/сброс" || message === "/reset") {
        await env.CHAT_HISTORY.delete(`history:${userId}`);
